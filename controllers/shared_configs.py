...
# ── 1v1 IPC channels ─────────────────────────────────────────────────────────
# Viper keeps channels 0/1 (same as 1v0).
# Titan uses channels 2/3 so broadcasts don't cross-contaminate.
# Titan.proto accepts action_channel/sensor_channel fields with defaults 0/1,
# so existing 1v0 worlds are fully backward compatible.
IPC_1V1 = {
    "viper_action_channel": 0,   # supervisor_emitter        → Viper receiver
    "viper_sensor_channel": 1,   # Viper emitter             → supervisor_receiver
    "titan_action_channel": 2,   # supervisor_emitter_titan  → Titan receiver
    "titan_sensor_channel": 3,   # Titan emitter             → supervisor_receiver_titan
    "action_fmt":   IPC["action_fmt"],
    "action_bytes": IPC["action_bytes"],
    "n_lidar":      IPC["n_lidar"],
    "sensor_fmt":   IPC["sensor_fmt"],
    "sensor_bytes": IPC["sensor_bytes"],
}
